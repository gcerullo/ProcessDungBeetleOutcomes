...
#read in functions
source(file.path(functions_dir, "DB_functions.R"))
...
Primary <- read.csv(file.path(raw_data_dir, "MastersPrimaryForestSites.csv")) %>% cbind(habitat = "primary") %>% rename(spp = 1) 
Logged <- read.csv(file.path(raw_data_dir, "LoggedMastersSites.csv")) %>% cbind(habitat = "once-logged")  %>% rename(spp = 1)
Restored <- read.csv(file.path(raw_data_dir, "MastersRestoredSites.csv")) %>% cbind(habitat = "restored") %>% rename(spp = 1)
#combine into a list
...
catherine <- read.csv(file.path(raw_data_dir, "DB_Data_2019_Finlayson_2L_and_Primary.csv")) %>% rename(spp =1) %>%  slice(-1:-5)


#Felicity's Data ####
#Nb - this is pooled across 4 days and must have Gianluca Master's and Trond sites removed 

trond_felicity <- read.csv(file.path(raw_data_dir, "FelicityTrondGianData.csv")) 

#Trond's Data ####
#read in Trond 2wice logged data (split by day)
trond_2l <- read.csv(file.path(raw_data_dir, "TwiceLoggedTrondData.csv")) %>% slice(-1:-5)

#Eleanor Slade's data ####
#Read in data from Eleanor Slades 2005 BRL and Danum PhD
slade <- read.csv(file.path(raw_data_dir, "Slade_2005_DanumBrL.csv")) %>%  rename(traptype = trap, 
                                                                 trap = trapID, 
...
gian2022 <- read.csv(file.path(raw_data_dir, "DungBeetle2022DataRawDanum_SSBPlantation.csv")) %>% mutate_all(as.character)
...
allsppNameBackbone <- read.csv(file.path(inputs_dir, "AllNamesBackbone.csv")) %>% dplyr::select(-X)

#GPS locations ####
#NB need to come back to this as lots of points are certain points are missing GPS data 

HistoricGPS <- read.csv(file.path(inputs_dir, "allHistoricSites.csv")) %>% dplyr::select(site,trap, Long, Lat)

#add gian2022 forest point count data 
GC2022_gps <- read.csv(file.path(inputs_dir, "allForest2022PointLocations.csv")) %>% rename(Long = X, Lat = Y) %>% 
...
slade_GPS <- read.csv(file.path(inputs_dir, "EleanorSladeDungBeetle_GPS_points.csv")) %>% select(lat, lon, GC_ID) %>% 
  rename(trap = GC_ID, 
         Long = lon, 
         Lat = lat)
...
#Plantation age info ####
plantation <- read.csv(file.path(inputs_dir, "Plantation_Habitat_Structure.csv")) %>% dplyr::select(Site,Point, Age) %>% 
...
plantationGPS <- read.csv(file.path(inputs_dir, "DaveSimonDanielPoints.csv")) %>%
...
correctIDsSingapore <- read.csv(file.path(raw_data_dir, "SingaporeIDCheckCrossWalk.csv")) %>%  
  rename() %>% 
  mutate(abundance = 1) %>%  
  rename(day = Day, 
         trap = Trap, 
...
write.csv(full_df, file.path(nr2_rds_dir, "dungBeetlesForAbundanceAnalysis.csv"))
...
#     TRUE ~ habitat))
# write.csv(anna_data, file.path(nr2_rds_dir, "data_for_annna_db_borneo.csv"))
