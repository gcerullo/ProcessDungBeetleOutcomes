...
df <- read.csv(file.path(nr2_rds_dir, "dungBeetlesForAbundanceAnalysis.csv"))
...
write.csv(sum_df, file.path(nr2_rds_dir, "full_DB_dataframeFor_BRMS_analysis_withSingletons.csv"))

#without singletons and doubletons 
df_filt <- sum_df %>% anti_join(singleton_doubleton_spp, by = "spp") 

write.csv(df_filt, file.path(nr2_rds_dir, "full_DB_dataframeFor_BRMS_analysis_withoutSingletonsAndDoubletons.csv"))
